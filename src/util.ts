...
export function chainBundle (transformSourceFile: (x: ts.SourceFile) => ts.SourceFile): ts.Transformer<ts.SourceFile> {
  ...
  function transformBundle (node: ts.Bundle) {
    return ts.createBundle(node.sourceFiles.map(transformSourceFile), node.prepends)
  }
}
