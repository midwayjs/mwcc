...
describe('comprehension', () => {
  it('should statically evaluate @Configuration', () => {
    const cli = findAndParseTsConfig(path.resolve(__dirname, 'comprehension-projects/midwayjs-decorator'))
  ...
  it('should statically evaluate @Configuration in JavaScript project', () => {
    const cli = findAndParseTsConfig(path.resolve(__dirname, 'comprehension-projects/midwayjs-decorator/node_modules/configuration-dep'), undefined, undefined, undefined, {
      compilerOptions: {
        allowJs: true,
    ...
    const program = ts.createProgram(cli.fileNames, cli.options, host)
    const result = parseConfiguration(program)
    assert.deepStrictEqual(result, { imports: [ { moduleId: 'foo' } ], configs: [] })
  })
})
